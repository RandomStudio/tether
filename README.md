...
You can use `tether send` in combination with `tether receive` to simulate a very simple Tether system. You can use the publicly-available MQTT broker `tether-io.dev:1883` by passing `--tether.host tether-io.dev` if you don't want the hassle of setting up your own MQTT Broker.
